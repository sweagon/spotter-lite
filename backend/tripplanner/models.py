...
class DutyEvent(models.Model):
    """a recorded duty-status change on a driver's record of duty status.
    exactly one event per driver is 'open' (ended_at=None) at a time — every
    change closes the previous and opens the next, so today's log sheet can be
    drawn straight from these rows in FMCSA order. the driver self-declares
    these; this is a planning log, not an engine-linked ELD record."""
    ...
    driver = models.ForeignKey(
        "Driver", on_delete=models.CASCADE, related_name="duty_events", db_index=True
    )
    ...
    trip = models.ForeignKey(
        "Trip", on_delete=models.SET_NULL, null=True, blank=True, related_name="duty_events"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def __str__(self):
        return f"{self.driver} {self.status}@{self.started_at:%H:%M}"
